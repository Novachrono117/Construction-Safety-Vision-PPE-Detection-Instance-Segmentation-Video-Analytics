...
O projeto também inclui um Google Colab executável. O **Mode A** apresenta arquitetura, métricas e figuras commitadas sem checkpoints, credencial de dataset ou GPU. O **Mode B** permite inferência opcional em vídeo externo, exigindo upload manual dos checkpoints congelados D2/S1 e verificação de tamanho e SHA-256 antes da desserialização. Ambos foram validados por execução humana no Colab; a validação de inferência utilizou Tesla T4 [8]. A entrega do notebook não equivale à reprodução integral da pesquisa desde um clone sem insumos.

O atendimento ao texto literal da atividade é **parcial**: faltam seções explícitas de treino e avaliação com suas saídas. O notebook publicado não executa treino nem avaliação final; apresenta resultados já registrados e inferência opcional. A correção de entrega proposta é separar receitas reprodutíveis e evidências históricas de execução atual, preservando o holdout. Se forem exigidas células que treinem efetivamente, será necessária decisão humana sobre reprodução opcional apenas em treino/validação. Essa pendência não foi resolvida durante a redação do relatório.
