...
O projeto inclui um Google Colab executável em sequência acadêmica: visão geral, dataset e classes, **treino**, **avaliação**, evidência qualitativa, evidência em vídeo, **inferência** e limitações. O notebook é pedagógico e explica em Markdown que D2 é o detector YOLO11n e S1 o segmentador YOLO11n-seg, por que cada um foi selecionado na validação, a inicialização por transferência de aprendizado a partir dos pesos pré-treinados, os hiperparâmetros e as augmentations registrados, a resolução efetiva de `optimizer: auto` para AdamW, os algoritmos de avaliação com mAP, IoU, precisão e recall, os pontos de operação e o NMS, o pipeline de inferência e a distinção entre caixas e máscaras. As células de treino leem o manifesto commitado de cada modelo congelado e exibem a receita completa, a regra de seleção de checkpoint, a época escolhida, a identidade congelada e as curvas registradas na época. As células de avaliação leem por campo os artefatos da avaliação única do holdout e exibem AP canônico, precisão e recall no ponto de operação, AP por classe, o diagnóstico direto de IoU de máscara, TP/FP/FN, as matrizes de confusão e a comparação validação-versus-teste. O **Mode B** permite inferência opcional em vídeo externo com os modelos congelados, exigindo upload manual dos checkpoints D2/S1 e verificação de tamanho e SHA-256 antes da desserialização.
...
Materiais da entrega, públicos na branch `main` — repositório e notebook executável:
