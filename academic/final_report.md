...
O projeto também inclui um Google Colab executável, organizado em sequência acadêmica: visão geral, dataset e classes, **treino**, **avaliação**, evidência qualitativa, evidência em vídeo, **inferência** e limitações. As células de treino leem o manifesto commitado de cada modelo congelado e exibem a receita completa - arquitetura, resolução, épocas, batch, seed, o otimizador ao qual `optimizer: auto` efetivamente resolveu, o conjunto de augmentations, a regra de seleção de checkpoint, a época escolhida e a identidade congelada - seguidas das curvas de treino e validação registradas na época. As células de avaliação leem por campo os artefatos commitados da avaliação única do holdout e exibem AP canônico, precisão e recall no ponto de operação, AP por classe, o diagnóstico direto de IoU de máscara, as contagens TP/FP/FN, as duas matrizes de confusão e a comparação validação-versus-teste. O **Mode B** permite inferência opcional em vídeo externo com os modelos congelados, exigindo upload manual dos checkpoints D2/S1 e verificação de tamanho e SHA-256 antes da desserialização.

O notebook contém, portanto, células executáveis de treino, avaliação e inferência com saídas visíveis. É preciso ser explícito sobre o que elas executam: as seções de treino e avaliação apresentam evidência registrada (`RECORDED_TRAINING_EVIDENCE` / `NO_NEW_TRAINING_EXECUTED`) e **não retreinam os modelos nem reexecutam a avaliação do holdout** por padrão; tampouco existe caminho executável que o faça. Cada modelo foi treinado uma única vez sob protocolo congelado antes da execução, e o holdout foi lido uma única vez e está gasto, de modo que reexecutar qualquer um invalidaria os resultados publicados. Os pontos de entrada reprodutíveis são impressos junto de cada receita, mas exigem GPU CUDA e o dataset materializado, que o notebook não fornece. A validação humana em Colab cobriu o Mode A em runtime CPU novo e o Mode B em Tesla T4 [8]; o Mode B não foi reexecutado após a inclusão das seções, pois todas as suas entradas de execução permanecem byte-idênticas. A entrega do notebook não equivale à reprodução integral da pesquisa desde um clone sem insumos.
...
[8] Gomes, Vinicius Pereira. **Construction Safety Vision: registros de experimentos, resultados e entrega**. [Repositório na revisão-fonte](https://github.com/Novachrono117/Construction-Safety-Vision-PPE-Detection-Instance-Segmentation-Video-Analytics/tree/a788d5303e70ddb12f5dc5ae35dd9a4b56fc6736). Os campos exatos, hashes e figuras utilizados neste relatório são discriminados no registro de proveniência que acompanha esta entrega.
