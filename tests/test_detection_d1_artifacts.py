...
def test_no_final_winner_is_declared_anywhere(manifest, report, results):
    assert results["selection"]["case"] is None
    assert results["selection"]["preferred_experiment"] is None
    assert results["selection"]["status"] == "PENDING_INCOMPLETE_MATRIX"
    lowered = report.lower()
    for phrase in ("phase 7 winner is", "final detector is d1", "d1 is the winner"):
        assert phrase not in lowered


def test_d2_remains_not_executed(results, paths):
    rows = {row["experiment_id"]: row for row in results["experiments"]}
    assert rows["D2"]["status"] == "FROZEN_NOT_EXECUTED"
    assert rows["D2"]["metrics"] is None
    assert rows["D2"]["margin_status"] is None
    assert "D2" in results["pending_experiments"]
    assert not (paths.reports / "detection_D2_manifest.json").exists()
    assert not (paths.root / "artifacts" / "detection" / "D2").exists()


def test_the_results_artifact_carries_d0_and_d1_only(results):
    rows = {row["experiment_id"]: row for row in results["experiments"]}
...
def test_no_placeholder_metric_was_invented_for_the_pending_experiment(results):
...
def test_the_report_states_that_d2_is_pending(report):
    assert "D2 remains pending" in report
