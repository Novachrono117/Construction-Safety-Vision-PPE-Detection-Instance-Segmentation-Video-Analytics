...
remains OPEN. The canonical notebook clones `main`; Phase 14A is now published.
The approved two-mode scope is historical: the newly supplied exact assignment
wording is **COLAB_EXACT_WORDING_PARTIAL** (training/evaluation sections and
visible recorded outputs need separate delivery work). Phase 14 is not complete.

**Academic technical report:** [Phase 14B](final_academic_report_delivery.md)
provides the Portuguese report in Markdown and a ten-page PDF. GAP-003/R15 are
complete. Test error commentary uses committed aggregates and deterministic
selection metadata; validation figures are labelled separately. The exact
assignment audit keeps Colab partial, pitch open and bonus not started.
Delivery planning targets 19 September, before the 20 September 2026 23:55 deadline.
...
| Date | Change |
| --- | --- |
| 2026-09-18 | Phase 15C hardened the Colab notebook pedagogically and re-validated it. Six new explanatory sections were added - 3.1 the two frozen models and why they were selected, 3.2 the recorded training hyperparameters, 3.3 augmentation, 4.1 how the evaluation is computed, 4.2 operating points and matching rules, and 7.1 how the inference pipeline works - and the human executed **Mode A** on a fresh Google Colab **CPU** runtime at revision `958eefaa58b785ff2f9eb4dd2751efc26dd9b09e` with `RUN_INFERENCE=False`, observing the correct revision, no traceback, TREINO rendering the recorded D2 and S1 evidence, AVALIACAO rendering the committed final-test evidence, Mode B installation skipped and the final line `Mode A complete. No inference output was created.` The clone target was changed inside the session from `main` to `phase15c-colab-pedagogical-validation`; that is a validation-session delta, not a repository defect, and the committed notebook still clones `main`. **Mode B was not re-executed**, and its phase 14A evidence is carried forward on two derived byte-identity proofs rather than an assertion: all 25 Mode B execution inputs are unchanged (`MODE_B_REVALIDATION_NOT_REQUIRED`), and the notebook's own 15 code cells are unchanged as whole objects since the phase 14C validated notebook (`NOTEBOOK_EXECUTABLE_SURFACE_UNCHANGED`, `MARKDOWN_ONLY`, Markdown cells 14 -> 20). That second proof exists because a Mode A run never enters the Mode B cells, so a Mode A pass alone could not validate them; the pre-existing phase 14C distance from the phase 14A notebook is disclosed and unchanged, not closed. The canonical notebook identity is now 34771 bytes / SHA-256 `5c92ff84289cf2a47cc5f5164826a099a5c43bf665ff8003f05bf254dc625dea`, and the phase 14A and 14C validation events are preserved beside the new one rather than overwritten. **No model was executed, no training or evaluation was run, no holdout byte was read, no metric was recomputed, no threshold or model selection changed and no scientific artifact changed.** |
