...
Version: 2.0 · Current phase: **8 - Segmentation baseline** (8A adapter fidelity audited · 8B YOLO11n-seg selected and the S0 protocol frozen · **8C complete: S0 trained once, mask mAP@0.50:0.95 0.407942 on validation, plus a predeclared direct instance-mask IoU diagnostic**). **No final segmenter is selected** - S0 is the only segmentation experiment. Phase 7 is complete: 7A-7C ran all three detection experiments and **7D froze the final detector**, D2 - YOLO11n at imgsz 768, selected on validation only.
...
| 8 | Segmentation baseline | complete (8A `SEGMENTATION_ADAPTER_AUDIT_COMPLETE` · 8B `S0_PROTOCOL_FROZEN`: YOLO11n-seg selected, adapter `APPROVED_FOR_CONTROLLED_TRAINING` · 8C `S0_SEGMENTATION_BASELINE_COMPLETE`: one run, mask mAP@0.50:0.95 0.407942, direct mask IoU measured · **final segmenter `UNSELECTED_PENDING_REVIEW`**) |
...
### Phase 8D - segmentation review and experiment planning (next, not started)

Human review of the S0 result, then a segmentation comparison protocol frozen
before the experiments it would decide exist - exactly as phase 7A did for
detection. No S1 may be trained before that protocol is recorded.
