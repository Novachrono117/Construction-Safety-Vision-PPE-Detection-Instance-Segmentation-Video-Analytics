...
Version: 2.0 · Current phase: **7 - Detection experiments and model freeze** (7A-7C complete: all three experiments run; **7D next** - final detector selection, deliberately not made)
...
| 7 | Detection experiments and model freeze | in progress (7A protocol · 7B D1 `BELOW_D0` · 7C D2 `IMPROVES_D0_BEYOND_MARGIN` · **7D next**: selection, `UNSELECTED_PENDING_REVIEW`) |
...
### Phase 7D - final detector selection (next, not started)

All three experiments have results, so the frozen Case A/B/C logic can be
evaluated; it computes `CASE_B_VALIDATION_PERFORMANCE_LEADER` with D2 leading.
That is recorded as an **advisory** `policy_case_candidate` only.
`final_selected_detector` is `UNSELECTED_PENDING_REVIEW`, D0 remains preferred
by default rather than by comparison, and no efficiency benchmark has been run.
Selecting and freezing the detector is this phase's job, after human review.
