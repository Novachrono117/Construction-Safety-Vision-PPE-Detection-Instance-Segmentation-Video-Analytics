...
| 8 | Segmentation baseline | complete (8A adapter audited · 8B `S0_PROTOCOL_FROZEN` · 8C `S0_SEGMENTATION_BASELINE_COMPLETE`: mask mAP@0.50:0.95 0.407942 · 8D `S1_HYPOTHESIS_READY_FOR_PROTOCOL_FREEZE`: `person` deficit largely a target-versus-evaluation mismatch · **final segmenter `UNSELECTED_PENDING_REVIEW`**) |
...
### Phase 8E - S1 protocol freeze (next, not started)

Human review of the 8D evidence, then a segmentation comparison protocol frozen
before the experiment it would decide exists - exactly as phase 7A did for
detection. It must settle which metric adjudicates an `overlap_mask` comparison,
since the framework's mask mAP is not comparable across that flag. No S1 may be
trained before that protocol is recorded.
