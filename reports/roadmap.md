...
Version: 2.0 · Current phase: **8 - Segmentation baseline** (8A complete: the YOLO segmentation-adapter fidelity audit is done; **8B complete: YOLO11n-seg selected, the audited adapter approved for controlled training and the S0 protocol frozen**; 8C, the S0 baseline run, is next and **S0 has not been trained**). Phase 7 is complete: 7A-7C ran all three detection experiments and **7D froze the final detector**, D2 - YOLO11n at imgsz 768, selected on validation only.
...
| 8 | Segmentation baseline | in progress (8A `SEGMENTATION_ADAPTER_AUDIT_COMPLETE`: format fidelity measured over all 1726 development instances · 8B `S0_PROTOCOL_FROZEN`: YOLO11n-seg selected, adapter `APPROVED_FOR_CONTROLLED_TRAINING`, S0 protocol frozen, smoke test passed · **8C next**: run S0 once, `NOT_EXECUTED_PROTOCOL_ONLY`) |
...
### Phase 8C - S0 segmentation baseline training (next, not started)

Run S0 exactly once under the frozen protocol. Do not tune, do not vary a
hyperparameter to see whether the number moves, and do not begin without an
