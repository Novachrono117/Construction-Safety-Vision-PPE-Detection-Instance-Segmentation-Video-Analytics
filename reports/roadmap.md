...
Version: 2.0 · Current phase: **8 - Segmentation baseline** (8A complete: the YOLO segmentation-adapter fidelity audit is done and **no architecture has been selected**; 8B, the human architecture decision, is next). Phase 7 is complete: 7A-7C ran all three detection experiments and **7D froze the final detector**, D2 - YOLO11n at imgsz 768, selected on validation only.
...
| 8 | Segmentation baseline | in progress (8A `SEGMENTATION_ADAPTER_AUDIT_COMPLETE`: format fidelity measured over all 1726 development instances · **8B next**: architecture decision, `UNSELECTED_PENDING_FIDELITY_REVIEW`) |
...
### Phase 8B - segmentation architecture decision (next, not started)

Human review of the 8A evidence decides the model path. No segmentation model
may be trained and no S0 protocol defined before that decision is recorded.
